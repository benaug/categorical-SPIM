...
source("NimbleModel catSPIM Poisson DA2.R")
source("NimbleFunctions catSPIM Poisson DA2.R")
source("sSampler.R")
...
#Simulate some data
data <- simCatSPIM(N=N,lam0=lam0,sigma=sigma,K=K,X=X,buff=buff,obstype="poisson", #this nimble model set up for poisson
...
parameters <- c('lambda.N','lam0','sigma','N','n','gammaMat')
...
conf$addSampler(target = paste0("y.true[1:",M,",1:",J,"]"),
                type = 'IDSampler',control = list(M=M,J=J,this.j=data$this.j,G.obs=data$G.obs,IDups=1,
...
for(i in 1:M){
  conf$addSampler(target = paste("s[",i,", 1:2]", sep=""),
                  type = 'sSampler',control=list(i=i,xlim=nimbuild$xlim,ylim=nimbuild$ylim,scale=0.33,adaptive=TRUE),silent = TRUE)
}
